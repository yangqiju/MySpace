package com.joyveb.thread.test;
...
/**
 * 
 * 项目名称：MySpace 类名称：ThreadTest
 * 
 * @Company: 北京畅享互联有限公司
 * @Copyright: Copyright (c) 2012
 * @Author： 杨其桔 创建时间：2013-9-18 下午1:44:49 修改备注：
 * @version
 * 
 */
public class CountDownLatchTest {
	...
	private static void waitStudentGetOn(CountDownLatch waitStudentsGetOn)
		...
		//waitStudentsGetOn.await();//将会一直等待,直到waitStudentsGetOn的值为0
	}
		...
		for (int i = 0; i < CountDownLatchTest.numberOfPeople; i++) {
			try {
				if (CountDownLatchTest.isGone) {
					System.out.println("妈的，还差：" + waitStudentsGetOn.getCount()
							+ " 个没娃上车呢.怎么车走了");
					break;
				}
			...
			if (waitStudentsGetOn.getCount() != 0l) {
				System.out.println("还差：" + (waitStudentsGetOn.getCount())
						+ " 个没上车");
			} else {
